  ...
  if (error instanceof z.ZodError) {
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      env = {
        ...
        NEXT_PUBLIC_ADMIN_EMAILS: process.env.NEXT_PUBLIC_ADMIN_EMAILS?.split(',').map(email => email.trim()) || [],
        LOG_LEVEL: 'info',
...
export const isAdminEmail = (email: string): boolean => {
  return env.NEXT_PUBLIC_ADMIN_EMAILS.includes(email.toLowerCase());
};
...
export const validateSecurityConfig = (): {
  ...
  // 관리자 이메일 설정 확인
  if (env.NEXT_PUBLIC_ADMIN_EMAILS.length === 0) {
...
export const logEnvironmentInfo = (): void => {
  const safeEnvInfo = {
    ...
    ADMIN_EMAILS_COUNT: env.NEXT_PUBLIC_ADMIN_EMAILS.length,
...
// 런타임에서 환경 변수 업데이트 (개발 환경에서만)
export const updateEnvVar = (key: keyof Env, value: string): boolean => {
  if (!isDevelopment()) {
    console.warn('환경 변수는 개발 환경에서만 업데이트할 수 있습니다.');
    return false;
  }

  try {
    const updatedEnv = { ...env, [key]: value };
    const validatedEnv = envSchema.parse({ ...process.env, [key]: value });
    
    // 검증 성공 시 환경 변수 업데이트
    env = validatedEnv;
    process.env[key] = value;
    
    console.log(`환경 변수 ${key} 업데이트됨`);
    return true;
  } catch (error) {
    console.error(`환경 변수 ${key} 업데이트 실패:`, error);
    return false;
  }
};
